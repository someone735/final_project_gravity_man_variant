...
class Platform(pygame.sprite.Sprite):
    def __init__(self, x, y, width, height, speed):
        ...
        """
        super().__init__()
        self.image_rightsideup = self.create_image(os.path.join("assets", "platform_tile.png"), width, height) # creates original image for platform
        self.image_upsidedown = pygame.transform.flip(self.image_rightsideup, False, True) # creates a copy of original platform image and flips 
        if y <= 400:
            self.image = self.image_upsidedown
        else:
            self.image = self.image_rightsideup
